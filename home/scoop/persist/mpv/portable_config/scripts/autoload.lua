...
-- Add at most 2000 * 2 files when starting a file (before + after).
...
function alphanumsort(filenames)
    ...
    for i, f in ipairs(filenames) do
        -- tuples[i] = {f:lower():gsub("0*(%d+)%.?(%d*)", padnum), f} -- load a001 after a001_2
        tuples[i] = {f:lower():gsub("0*(%d+)(%d*)", padnum), f} -- load a001_2 after a001
    end
